...
public class NearbyPermutationEngineTest {
	...
	@Before
	public void setUp() {
		pe = new NearbyPermutationEngine<String>(new Random(6)); // chosen by fair dice roll
	}
	...
	@Test
	public void testWithTwoListProbabilityReturnsDistinctValue() {
		List<String> twolist = ImmutableList.of("foo", "bar");
		List<String> result = pe.next(twolist);
		Assert.assertEquals(result, ImmutableList.of("bar", "foo"));
	}
	@Test
	public void testWithThreeListProbabilityLooksRight() {
		List<String> twolist = ImmutableList.of("foo", "bar", "baz");
		int iter = 10000;
		int count = 0;
		...
		// The algorithm for calculating a "nearby" permutation randomly chooses a slot and swaps it.
		// This works out to being a 1/3 chance of swapping the first item.
		...
		// As of the time of this writing, the actual ratio is 0.3353 (on JDK8)
		Assert.assertTrue(count > (1.0 * iter * 0.31));
		Assert.assertTrue(count < (1.0 * iter * 0.35));
	}
}
